...
public:
  ...
  Player * find_min(std::vector<Player*> players , std::vector<int> mins);
  
  bool is_in_mins(std::vector<int> mins , int id);
  
  std::vector<Coordinate>  valid_coords_attack_inboard(std::vector<Coordinate> coords,Coordinate self);
  std::vector<Coordinate>  valid_coords_move(std::vector<Coordinate> coords,Coordinate self);
  
  int find_min_distance(std::vector<int> distances);
  
  std::vector<Player*>* kik_same_team(std::vector<Player*>* players,Team team);
  
  Coordinate find_uygun_cord(Coordinate min_id_cord, std::vector<Coordinate> move_coords, Coordinate self);
  
  void delete_in_game_players(Player * player);

  std::vector<int> delete_from_mins(std::vector<int> mins,int id);
  std::string control_id(uint id);
  bool is_one_digit(uint id);
