...
public:
    ...
    // ADD OWN PUBLIC METHODS/PROPERTIES BELOW
     void goBacktoLap_helper(Car *car,int lap);
     void add_one_more_lap(Car *car);
     void remove_one_more_lap(Car *car);
     int give_car_number(Car *head);
     Car * find_min_prev(Car * old_head);
     Car * find_min(Car * old_head);
     int find_total_laptime(Car * temp);
     void temizle(Car * head);
